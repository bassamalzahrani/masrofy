// Masrofy v1.9 - Smart Expense Tracker (100% client-side, localStorage)
...
let state = { type: 'expense', txs: [], budget: 0, currency: 'ر.س', editingId: null, plan: null, accounts: [{ id: 'cash', name: 'كاش', icon: '💵' }], challenges: [], lock: { enabled: false, pin: '' } };
...
const cleanCategory = (id, type) => CATEGORIES[type].some(c => c.id === id) ? id : (type === 'income' ? 'other-inc' : 'other-exp');
...
function load() {
  ...
  if (!Array.isArray(state.accounts) || !state.accounts.length) {
    state.accounts = [{ id: 'cash', name: 'كاش', icon: '💵' }];
  }
  ...
  if (!state.accounts.length) state.accounts = [{ id: 'cash', name: 'كاش', icon: '💵' }];
  state.txs.forEach(t => {
...
function save() {
  try {
    localStorage.setItem('masrofy-v1', JSON.stringify({ txs: state.txs, budget: state.budget, currency: state.currency, plan: state.plan, accounts: state.accounts, challenges: state.challenges, lock: state.lock }));
    return true;
  } catch {
...
const accBalance = (id) => state.txs.filter(t => String(t.accountId) === String(id)).reduce((s, t) => s + (t.type === 'income' ? t.amount : -t.amount), 0);
...
$('importFile').onchange = (e) => {
  ...
  reader.onload = () => {
    try {
      ...
      const sourceAccounts = !Array.isArray(data) && Array.isArray(data.accounts) && data.accounts.length ? data.accounts : [{ id: 'cash', name: 'كاش', icon: '💵' }];
      const accountIds = new Set();
      const accounts = sourceAccounts.filter(a => a && a.id != null).map(a => ({ id: safeText(a.id, 64), name: safeText(a.name || 'حساب', 20), icon: safeText(a.icon || '💵', 4) }))
        .filter(a => a.id && !accountIds.has(a.id) && accountIds.add(a.id));
      if (!accounts.length) accounts.push({ id: 'cash', name: 'كاش', icon: '💵' });
      const ids = new Set();
      const clean = sourceTxs.map(t => {
        ...
        const row = { id, type: t.type === 'income' ? 'income' : 'expense', amount: Number(t.amount),
          category: cleanCategory(t.category, t.type === 'income' ? 'income' : 'expense'), date: String(t.date).slice(0, 10), note: safeText(t.note), recur,
...
function renderList() {
  ...
  box.innerHTML = list.map(t => `
    <div class="tx" data-tx-id="${escapeHtml(t.id)}">
      <div class="tx-info">
        <div class="tx-emoji" style="background:${CAT_GRAD[t.category] || 'var(--border)'}">${EMOJI[t.category] || '📌'}</div>
...
function fillAccounts() {
  ...
  const opts = state.accounts.map(a => `<option value="${escapeHtml(a.id)}">${escapeHtml(a.icon)} ${escapeHtml(a.name)}</option>`).join('');
  txSel.innerHTML = opts;
  fSel.innerHTML = '<option value="">كل الحسابات</option>' + opts;
  from.innerHTML = opts;
  to.innerHTML = opts;
  ...
  if (state.editingId) {
    const tx = state.txs.find(t => t.id === state.editingId);
    if (tx && has(txSel, tx.accountId)) txSel.value = tx.accountId;
  }
...
function renderAccList() {
  ...
  $('accList').innerHTML = state.accounts.map((a, i) => {
    ...
    return `<div class="acc-chip${isSel ? ' sel' : ''}" data-account-id="${escapeHtml(a.id)}" style="background:${ACC_GRAD[i % ACC_GRAD.length]}">
      <div class="acc-top"><span class="acc-ic">${escapeHtml(a.icon)}</span>${isSel ? '<span class="acc-check">✓ محدد</span>' : ''}<button class="acc-del" title="حذف الحساب">✕</button></div>
      <div class="acc-bal">${fmt(bal)}</div>
      <div class="acc-meta"><b>${escapeHtml(a.name)}</b><small>${n} عملية</small></div>
    </div>`;
  }).join('');
...
$('addAccBtn').onclick = () => {
  ...
  state.accounts.push({ id: String(Date.now()), name, icon: $('accIcon').value || '💵' });
  $('accName').value = '';
  save(); render();
...
window.accDel = (id) => {
  ...
  const acc = accOf(id);
  const replacement = state.accounts.find(a => String(a.id) !== String(id));
  const used = state.txs.filter(t => String(t.accountId) === String(id)).length;
  if (used && !confirm(`حساب "${acc.name}" فيه ${used} عملية — سيتم نقلها إلى "${replacement.name}". متابعة؟`)) return;
  if (!used && !confirm(`حذف حساب "${acc.name}"؟`)) return;
  state.txs.forEach(t => { if (String(t.accountId) === String(id)) t.accountId = replacement.id; });
...
$('trBtn').onclick = () => {
  ...
  state.txs.unshift({ id: base + 1, type: 'income', amount, category: 'other-exp', date, note: `تحويل من ${accName(from)} 🔄`, accountId: to, recur: 'none', transfer: true, transferId });
  $('trAmount').value = '';
  if (navigator.vibrate) navigator.vibrate(20);
  save(); render();
  toast('تم التحويل 🔄');
